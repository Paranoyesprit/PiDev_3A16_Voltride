...
public class afficherReservationController {
    ...
    private void openReservationView(Voiture voitureToReserver) {
        try {
            ...
            Controllers.ajouterReservationController controller = loader.getController();
    ...
    public void consulterReservation(ActionEvent actionEvent) {
        try {
            // Charger le fichier FXML correspondant à la vue de consultation
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/consulterListReservation.fxml"));
            Parent root = loader.load();

            // Obtenir le contrôleur associé à la vue chargée
            consulterListReservationController controller = loader.getController();
            ...
            // Appeler la méthode initialize du contrôleur une fois que la vue est chargée
            controller.initialize();
        } catch (IOException e) {
            e.printStackTrace(); // Gérer l'exception en fonction de votre cas d'utilisation
        }
    }
